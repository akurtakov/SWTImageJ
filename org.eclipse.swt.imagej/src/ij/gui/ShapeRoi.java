...
public class ShapeRoi extends Roi {
	 ...
	 * <td>Roi.POLYGON</td>
	 * <td>java.awt.Polygon or (if subpixel resolution) closed java.awt.geom.GeneralPath</td>
	 * </tr>
	 * <tr>
	 * <td>ij.gui.PolygonRoi</td>
	 * <td>Roi.FREEROI</td>
	 ...
	 * 
	 * @param nSegments
	 *            The number of segments that compose the path (= number of vertices for a polygon)
	 * @param polygonLength
	 *            length of polygon in pixels, or NaN if curved segments
	 * @param horizontalVerticalIntOnly
	 *            Indicates whether the GeneralPath is composed of only vertical and horizontal lines with integer coordinates
	 * @param forceTrace
	 *            Indicates that closed shapes with <code>horizontalVerticalIntOnly=true</code> should become TRACED_ROIs
	 * @param closed
	...
	double cplength(CubicCurve2D.Double c) {
		...
		return Math.sqrt(sqr(c.ctrlx1 - c.x1) + sqr(c.ctrly1 - c.y1)) + Math.sqrt(sqr(c.ctrlx2 - c.ctrlx1) + sqr(c.ctrly2 - c.ctrly1)) + Math.sqrt(sqr(c.x2 - c.ctrlx2) + sqr(c.y2 - c.ctrly2));
	}
	...
	double qplength(QuadCurve2D.Double c) {
		...
		return Math.sqrt(sqr(c.ctrlx - c.x1) + sqr(c.ctrly - c.y1)) + Math.sqrt(sqr(c.x2 - c.ctrlx) + sqr(c.y2 - c.ctrly));
	}
	 ...
	 * @param n
	 *            number of values in <code>c</code> that should be modified (must be less or equal to the size of <code>c</code>
	 *
	 */
	 ...
	 * <br>
	 * - add each subpath as a Roi to rois; curved subpaths will be flattened, i.e. converted to a
	 * polygon approximation (task != ONE_ROI and rois non-null)
	 * <br>
	 * - measure the combined length of all subpaths/Rois and return it (task = GET_LENGTH, rois may be null)
	 * 
	 * @param pIter
	 *            the PathIterator to be parsed.
	 * @param params
	 *            an array with one element that will hold the calculated total length of the rois if its initial value is 0.
	 ...
	 * @param rois
	 *            an ArrayList that will hold ij.gui.Roi objects constructed from subpaths of this path;
	 ...
	 * 
	 * @param flatness
	 *            Roughly the maximum allowable distance between the shape and the approximate polygon
	 * @param separateSubpaths
	 *            whether individual subpaths should be separated by NaN coordinates
	 * @param addPointForClose
	 ...
	 * @param absoluteCoord
	 *            specifies whether the coordinates should be with respect to image bounds, not Roi bounds.
	 */
