...
public class PointRoi extends PolygonRoi {
	...
	/**
	 * Creates a one-element array with a coordinate; if 'imp' is non-null converts
	 * from a screen coordinate to an image (offscreen) coordinate. The array can be
	 * used for adding to an existing point selection
	 */
	...
	public void draw(Graphics g) {
		...
		if (ic != null && overlay && ic.getShowAllList() != null && ic.getShowAllList().contains(this)
			...
			slice = 0; // In RoiManager's "show all" mode and not "associate with slice", draw point
	...
	private synchronized void incrementCounter(ImagePlus imp) {
		...
		if (counter != 0 || isStack || counters != null) {
			...
			// IJ.log("incrementCounter: "+nPoints+" "+counters.length+"
			// "+counters[nPoints-1]+" "+positions[nPoints-1]+" "+imp);
		}
		if (rt != null && WindowManager.getWindow(getCountsTitle()) != null)
			displayCounts();
	}
	 ...
	 * ROI. Returns null if there are no resulting points or Roi is not an area roi.
	 */
	...
	/**
	 * Returns an array containing for each point: The counter number (0-100) in the
	 * lower 8 bits, and the slice number (or 0, if the point appears on all slices)
	 * in the higher 24 bits. Used when writing a Roi to file (RoiEncoder)
	 */
	...
	/**
	 * Sets the stack position (image number) of all points in this Roi. The points
	 * are only displayed when the stack is at the specified position. Set to zero
	 * to have the points displayed on all images in the stack. The stack position,
	 * when set, determines the visibility of this Roi (i) if it is part of an
	 ...
	 * if that Roi is part of an overlay.
	 */
	...
	/**
	 * Returns the stack position (image number) of the points in this Roi, if all
	 * points have the same position. Returns 0 if none of the points is associated
	 * with a particular stack image, and PointRoi.POINTWISE_POSITION = -2 if there
	...
	/**
	 * Returns whether this Roi contains a point associated to the given stack
	 * slice. Returns true for (non-existant) slice 0. Does not care whether points
	 * would be shown irrespective of the slice number (as given by the Point Tool
	 * options "Show on all slices", Prefs.showAllPoints).
	 */
	...
	/**
	 * Returns a point index if it has been at least one second since the last point
	 * was added and the specified screen coordinates are inside or near a point,
	 * otherwise returns -1.
	 */
