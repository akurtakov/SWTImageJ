...
public class PolygonRoi extends Roi {
	...
	private void drawStartBox(Graphics g) {
		...
		if (type != ANGLE) {
			if (ic.isFitToParent()) {
				...
				g.drawRect((screenXD((startXD * ic.aspectRatioX) - 4)), (screenYD((startYD * ic.aspectRatioY) - 4)), 8,
						8);
			} else {
	...
	public void draw(Graphics g) {
		...
		if (xSpline != null) {
			...
			if (type == POLYLINE || type == FREELINE || type == ANGLE
					|| (state == CONSTRUCTING && !(this instanceof RotatedRectRoi))) {
	...
	protected void handleMouseUp(int sx, int sy) {
		...
		if (nPoints > 2 && (biggerStartBox.contains(sx, sy)
				|| (offScreenXD(sx) == startXD && offScreenYD(sy) == startYD) || (samePoint && doubleClick))) {
