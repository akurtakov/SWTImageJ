...
public class ColorProcessor extends ImageProcessor {
	...
	public static final int RGB_NOISE = 0, RGB_MEDIAN = 1, RGB_FIND_EDGES = 2,
			...
			RGB_SCALE = 7, RGB_RESIZE = 8, RGB_TRANSLATE = 9, RGB_MIN = 10,
			RGB_MAX = 11;
	...
	public void convolve3x3(int[] kernel) {
		...
		int k1 = kernel[0], k2 = kernel[1], k3 = kernel[2], k4 = kernel[3],
				k5 = kernel[4], k6 = kernel[5], k7 = kernel[6], k8 = kernel[7],
				k9 = kernel[8];
		int scale = 0;
	...
	private void blurMore() {
		...
		for(int y = roiY; y < roiY + roiHeight; y++) {
			...
			for(int x = roiX; x < roiX + roiWidth; x++) {
				...
				int rsum = (p1 & 0xff0000) + (p2 & 0xff0000) + (p3 & 0xff0000) + (p4 & 0xff0000) + (p5 & 0xff0000) + (p6 & 0xff0000) + (p7 & 0xff0000) + (p8 & 0xff0000) + (p9 & 0xff0000);
				int gsum = (p1 & 0xff00) + (p2 & 0xff00) + (p3 & 0xff00) + (p4 & 0xff00) + (p5 & 0xff00) + (p6 & 0xff00) + (p7 & 0xff00) + (p8 & 0xff00) + (p9 & 0xff00);
				int bsum = (p1 & 0xff) + (p2 & 0xff) + (p3 & 0xff) + (p4 & 0xff) + (p5 & 0xff) + (p6 & 0xff) + (p7 & 0xff) + (p8 & 0xff) + (p9 & 0xff);
